...
| Area | Status | Notes |
| --- | --- | --- |
| Core workbook/sheet/cell flows | Done | Create, add, set, remove, get, query, view, raw/raw-set, import, resident-backed mutation flows, and metadata-free OOXML mutation are in place. |
| Advanced Excel object paths | Mostly done | Named ranges, validations, conditional formatting, comments, tables, sparklines, charts, pivots, shapes, pictures, and chart add-part flows all have working adapter paths. |
...
| Query and view surface | Mostly done | `outline`, `text`, `annotated`, `stats`, `issues`, `html`, `json`; object queries for many Excel node families; resident-backed `get/query/view` flows | More OfficeCLI-like selector/filter semantics and deeper query detail for some object families | Medium |
...
| Formula evaluator (`FormulaEvaluator`) | Partial | Display-time evaluation for a growing high-frequency function set and cross-sheet references; current support includes `SUM`, `AVERAGE`, `MIN`, `MAX`, `COUNT`, `IF`, `COUNTA`, `SUMPRODUCT`, `INDEX`, `MATCH`, `VLOOKUP`, `HLOOKUP`, `SUMIF`, `COUNTIF`, `AVERAGEIF`, `SUMIFS`, `COUNTIFS`, `AVERAGEIFS`, `MAXIFS`, `MINIFS`, selected text helpers (`LEN`, `TRIM`, `UPPER`, `LOWER`, `LEFT`, `RIGHT`, `MID`, `CONCATENATE`, `TEXTJOIN`, `SEARCH`, `SUBSTITUTE`, `TEXT`, `FIXED`, `NUMBERVALUE`, `SWITCH`, `CHOOSE`), selected statistical/logical helpers (`MEDIAN`, `MODE`, `PERCENTILE`, `PERCENTRANK`, `COUNTBLANK`, `PRODUCT`, `QUOTIENT`), selected financial helpers (`PMT`), and selected date/time helpers (`DATE`, `YEAR`, `MONTH`, `DAY`, `DAYS`, `DATEDIF`, `NETWORKDAYS`, `NETWORKDAYS_INTL`, `WORKDAY`, `WORKDAY_INTL`, `YEARFRAC`, `EOMONTH`) | Large remaining function matrix still includes many OfficeCLI areas: broader financial/conversion coverage, more text/date/time/logical/statistical helpers, more lookup edge cases, more nested-expression support, richer Excel-style coercion/error propagation, error-array semantics, and more robust circular/lookup behavior | Very high |
| Style manager (`ExcelStyleManager`) | Partial | Generated styles for common `font.*`, `fill`, `numFmt`, `alignment.*`, selected border fields, and basic protection flags; style reuse for equivalent style props; styles.xml persisted and reused | Broader style surface still needed: more border variants and per-side nuance, more alignment fields/merge behavior, more font/fill combinations, better inheritance against existing styles, and stronger dedupe/style preservation semantics | Very high |
| Metadata-free OOXML compatibility | Mostly done | Real external OOXML files can be mutated without losing many existing workbook/style/formula features; OOXML text-template merge now works for workbook/sharedStrings-based templates | More pressure-testing across complex third-party workbooks mixing chart + pivot + drawing + CF + style + formula behavior | High |
...
| --- | --- | --- | --- |
| 1 | Formula evaluator | Directly affects `get`, `view`, and user trust in Excel read behavior | Push beyond the current lookup/conditional-aggregation baseline into more statistical/date/time/text/logical functions and more robust nested/error semantics |
...
| 3 | Style manager depth | Needed for reliable formatting parity, especially on real-world workbooks | More border variants, more alignment/protection nuance, richer font/fill combinations, and inheritance/merge logic against existing styles |
